export const softBlack = '#131313';

export const softWhite = '#dddddd';
export const darkGray = '#252525';
export const darkerGray = '#202020';
export const gray = '#797979';
export const lightGray = '#c7c7c7';
export const lighterGray = '#eaeaea';
export const blue = '#0653c0';
export const darkBlue = '#043483';
export const lightViolet = '#bb85fc';
export const darkViolet = '#75579f';
export const owlPink = '#ff0266';

export const gradients = {
  moonlitAsteroidGradient: 'linear-gradient(90deg, #0f2027 0%, #203a43 48%, #2c5364 100%);',
  royal: 'linear-gradient(90deg, rgba(20,30,48,1) 0%, rgba(36,59,85,1) 100%)',
  deepSpace: 'linear-gradient(180deg, #000000 0%, #191919 100%)',
  blue: 'linear-gradient(180deg, #02149c 0%,#298ACF 100%)',
  red: 'linear-gradient(180deg, #d10000 0%, #d27e7e 100%);',
  gray: 'linear-gradient(180deg, #f3f3f3 0%, white 100%)'
};
