...
const TopMenu = observer(() => {
  ...
  const renderLoadingAuth = () => {
    return playerInfoStore.isLoading ? (
  ...
  const renderMiniProfile = () => {
    return !playerInfoStore.isAuthorized ? (
      <TextRow as={NavLink} to="/admin/Auth">
        Войти
      </TextRow>
    ) : (
      <>
        <TextRow as={NavLink} to={`/admin/profile/${login}`}>
          {playerInfoStore.profile.login}
        </TextRow>
        <Avatar>{login}</Avatar>
        <TextRow
          as={NavLink}
          to="/admin"
          onClick={() => playerInfoStore.logOut()}
        >
          Выйти
        </TextRow>
      </>
